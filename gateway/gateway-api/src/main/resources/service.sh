...
#java虚拟机启动参数
JAVA_OPTS="$OPTS_PARAM -Dspath=$APP_FULL_NAME -XX:ErrorFile=./java_err_pid.log"
#当前应用的进程id
PS_ID=0
#当前停止次数
CUR_STOP_TIMES=0
...
startFR() {
   ...
   if [ $PS_ID -ne 0 ]; then
      ...
      echo -n "Starting $APP_NAME ..."
      java $JAVA_OPTS -jar $APP_FULL_NAME > /dev/null 2>&1 &
   fi
}
...
case "$1" in
   ...
   'startFR')
     ...
     ;;
  *)
     echo "Usage: $0 {start|stop|stopF|restart|restartF|status|info|startFR}"
     exit 1
esac
exit 0
