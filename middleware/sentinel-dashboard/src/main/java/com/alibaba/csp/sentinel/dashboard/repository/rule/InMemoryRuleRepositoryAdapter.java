...
public abstract class InMemoryRuleRepositoryAdapter<T extends RuleEntity> implements RuleRepository<T, Long> {
    ...
    public List<T> saveAll(List<T> rules) {
        ...
        //重置id，避免重启后造成id重复从而导致某些配置被覆盖的情况
        long maxId = calcMaxId(rules);
        resetId(maxId);
    ...
    private long calcMaxId(List<T> rules){
        long maxId = 0;
        ...
        for (T rule : rules) {
            if(rule.getId() != null && rule.getId() > maxId){
                maxId = rule.getId();
            }
        }
        return maxId;
    ...
    abstract protected void resetId(long id);
}
