...
public class CronExpressionUtil {
     ...
     *
     * 注意：
     *      Quartz的cron任务不能精确处理秒级别，此时请改用DailyTimeIntervalTrigger等方式来处理，怎么理解呢？假设有一个每25秒执行一次的任务
     *      正常理解是在当前这分钟内的第 25秒、50秒 各执行一次，然后在下一分钟的第 15秒执行一次，但cron任务不是这样，它会在下一分钟的0秒就执行一次，
     *      这就跟我们预期的不一致了，所以，cron任务适合间隔时间大于等于1分钟以上的任务，如果有每多少秒执行一次任务，请使用'间隔任务'来实现
     * @param seconds
     * @return
     */
    @Deprecated
    ...
    public static void main(String[] args){
        System.out.println(CronExpressionUtil.getSecondlyExpression(25));
        System.out.println(CronExpressionUtil.getMinutelyExpression(1));
    }
}
