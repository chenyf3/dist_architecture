...
class RSAUtil
{
    ...
    public static function encrypt(string $data, string $pubKey): string
    {
        ...
        if($pubKey === false){
            throw new SDKException(SDKException::BIZ_ERROR, "rsa加密公钥无效");
        }
     ...
     * @throws SDKException
     */
    public static function sign(string $data, string $priKey): string
    {
        $priKey = openssl_get_privatekey($priKey);
        if($priKey === false){
        ...
        $binary_signature = '';
        $isSuccess = openssl_sign($data, $binary_signature, $priKey, OPENSSL_ALGO_SHA256);
        openssl_free_key($priKey);
        if(! $isSuccess){
     ...
     * @throws SDKException
     */
    public static function verify(string $signData, string $signParam, string $pubKey): bool
    {
        $pubKey = openssl_get_publickey($pubKey);
        if($pubKey === false){
        ...
        $isMatch = openssl_verify($signData, $signParam, $pubKey, OPENSSL_ALGO_SHA256) === 1;
        openssl_free_key($pubKey);
        return $isMatch;
    }
}