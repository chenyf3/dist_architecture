...
public class DemoController {
    ...
    public String doUpload(@RequestParam("bucketName") String bucketName, @RequestParam("uploadFile") MultipartFile file) {
        ...
        String objectName;
        try {
            String bucketFullName = objectStorage.getBucketFullName(bucketName);
            objectName = objectStorage.putObject(bucketFullName, oriFileName, file.getBytes(), contentType);
        } catch(Exception e) {
            e.printStackTrace();
            return "error";
        }
    ...
    public String doUpload2(@RequestParam("bucketName") String bucketName, @RequestParam("uploadFile") MultipartFile file) throws Exception {
        ...
        while (current++ <= max) {
            try {
                ...
                String bucketFullName = objectStorage.getBucketFullName(bucketName);
                String objectFullName = objectStorage.getObjectFullName(bucketFullName, objectName);
                objectStorage.putObject(bucketFullName, objectFullName, data, contentType, null, null);
                success ++;
            } catch(Exception e) {
                fail ++;
                e.printStackTrace();
    ...
    public void getPicPage(String fileName, HttpServletResponse response) {
        ...
        int offset = 0;
        int pageSize = 102400;//每次分页获取的字节数，100KB
        byte[] allBuffer = null;
        boolean isGoOn = true;
        while (isGoOn) {
