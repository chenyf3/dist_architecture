    ...
    <div class="app-container">
        <div class="filter-container">
            <div class="filter-item">
                <el-button :loading="loading" class="el-button-small" @click="refreshAuthList" type="primary"><i class="el-icon-search"/>刷新
                </el-button>
                <el-button v-authorize="'portal:auth:add'" class="el-button-small" @click="addAuthPage({id:0, name:'根节点'})" type="success" plain><i
                    class="el-icon-plus"/>添加顶级菜单
                </el-button>
            </div>
        </div>
        <div class="">
            <el-table :data="treeData" border row-key="id"
                ...
                <el-table-column label="权限类型" prop="authType"
                                 :formatter="row => (row.authType === 1 ? '菜单' : '功能')"/>
                <el-table-column v-slot="{row}" label="操作">
                    <el-button v-authorize="'portal:auth:edit'" type="text" size="mini"
                               @click="editAuthPage(row)">编辑
                    </el-button>
                    <el-button v-authorize="'portal:auth:add'" v-if="row.authType===1" type="text" size="mini"
                               @click="addAuthPage(row)">添加权限
                    </el-button>
                    <el-button v-authorize="'portal:auth:delete'" type="text" size="mini"
...
import {listAllAuth, deleteAuth} from '@/api/portal/auth';
...
export default {
    ...
    methods: {
        ...
        deleteAuth(row) {
            this.$confirm('确定删除 [' + row.name + '] 这个权限节点吗?')
