        ...
        <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="100px">
            ...
            <el-form-item prop="receivers" label="收件人">
                <el-input v-model="form.receivers" type="textarea" :autosize="{minRows: 6, maxRows: 9}"
                          :readonly="actionType === 'VIEW'" placeholder="多个收件人请按回车换行"/>
            </el-form-item>
            <el-form-item prop="cc" label="抄送人">
                <el-input v-model="form.cc" type="textarea" :autosize="{minRows: 6, maxRows: 9}"
                          :readonly="actionType === 'VIEW'" placeholder="多个抄送人请按回车换行"/>
            </el-form-item>
...
<script>
import {addMailGroup, editMailGroup, getMailSender} from '@/api/baseConfig/mailGroup';
        ...
        doAdd() {
            this.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    addMailGroup(formData).then(resp => {
                        if(resp.code === 200){
        ...
        doEdit() {
            this.$refs.form.validate(valid => {
                if (valid) {
                    this.loading = true;
                    const formData = {...this.form};
                    editMailGroup(formData).then(resp => {
                        if(resp.code === 200){
                            this.$message.success(resp.data);
                            this.closeForm();
                            this.$emit('success');
